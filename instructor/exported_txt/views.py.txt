...
# ---------------------------------------------------
#  Instructor Dashboard (دوره‌ها، جلسات، آمار)
# ---------------------------------------------------
class InstructorDashboardViewSet(viewsets.ViewSet):
    ...
    def duplicate(self, request, pk=None):
        ...
        original = self._get_course(user, pk)
        if not original:
            return Response({"error": "Course not found"}, status=404)
        ...
        new_course = Course.objects.create(
            ...
            slug=slugify(f"{original.title}-copy-{original.id}"),
...
# ---------------------------------------------------
#  Instructor Video Dashboard (آپلود چانکد حرفه‌ای)
# ---------------------------------------------------
class InstructorvideoDashboardViewSet(viewsets.ViewSet):
    permission_classes = [IsAuthenticated]
    ...
    def upload_init(self, request, pk=None):
        ...
        upload_id = f"lesson_{lesson.id}_user_{user.id}"
    ...
    class InstructorvideoDashboardViewSet(viewsets.ViewSet):
        ...
        def upload_finalize(self, request):
            ...
            if not upload_id or not lesson_id or total_chunks is None:
                return Response({"error": "Missing finalize data"}, status=400)
            ...
            final_dir = os.path.join(settings.MEDIA_ROOT, "course_videos")
            os.makedirs(final_dir, exist_ok=True)

            local_final_path = os.path.join(final_dir, f"lesson_{lesson.id}.mp4")

            # MERGE CHUNKS
            with open(local_final_path, "wb") as final_file:
            ...
            # اگر فعلاً استوریج لوکال است:
            # مستقیماً روی فیلد مدل ست می‌کنیم
            with open(local_final_path, "rb") as f:
                lesson.video_file.save(f"lesson_{lesson.id}.mp4", File(f), save=True)

            # پردازش ویدئو در بک‌گراند (FFmpeg)
            process_lesson_video_task.delay(lesson.id, local_final_path)

            # اگر استوریج ریموت (S3) فعال است و بخوای ویدئو بره اونجا:
            # remote_path = f"course_videos/lesson_{lesson.id}.mp4"
            # upload_lesson_video_to_s3_task.delay(lesson.id, local_final_path, remote_path)

            return Response({
                "message": "Upload completed",
                "file": lesson.video_file.url
            })
